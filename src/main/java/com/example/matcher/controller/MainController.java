...
public class MainController {
    ...
    public String uploadAndGetSuggestions(final @RequestParam("file") MultipartFile file, Model model) {
        ...
        try {
            matchingResult = matchingService.calculateBestMatchingCouplesFromCsv(file);
        ...
        } catch (final UnprocessableCsvFileException e) {
            model.addAttribute("errorMessage", "Unable process provided file");
            return "index";
        } catch (WrongFileExtensionException e) {
            model.addAttribute("errorMessage", "Please provide a valid .cvs file");
            return "index";
        }
        return "suggestion";
    }
